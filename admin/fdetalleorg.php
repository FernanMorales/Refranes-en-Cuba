    ...
    <td width="765"><div id="main">
	 <div id="contenido">
	  <?php include('menu.php') ?>
	   <div id="titulo">Detalle origen 
	     <div id="logout"><a href="logout.php" class="elemento logout">Salir</a></div></div>
	   <div id="cajaizq">	   
<?php
if (isset($_GET['refid'])){
$refid = $_GET['refid'];
$cadref = "SELECT * FROM origen WHERE id = ".$refid;
$resref = $db->sql_query($cadref);
$des = $db->sql_fetchfield('des', 0, $resref);
echo '<div class="titulo">Origen</div>';
echo '<div id="detalles">'.$des.'</div>';
}else{
	echo '<br />Ocurrio un problema en la adición de este origen. Intentelo nuevamente.';
	echo '<div id="refran1" style="height:500px"><a href="faddorigen.php" class="elemento">Adicionar origen</a></div>';
}
?>
	   </div>	  
	 </div>
