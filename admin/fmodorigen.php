    ...
    <td><div id="main">
	 <div id="contenido">
	  <?php include('menu.php') ?>
	   ...
	   <div id="cajaizq">
	     <form action="modorg.php" method="post" name="form" id="form">
	       <div class="titulo">Origen</div>
		   <div id="cajainsert">
<?php
if (isset($_GET['ref'])){
	$id = $_GET['ref'];
	echo '<input name="orgid" type="hidden" value="'.$id.'" />';
	$cadref = "SELECT * FROM origen WHERE id = ".$_GET['ref'];
	$resref = $db->sql_query($cadref);
	$des = $db->sql_fetchfield('des', 0, $resref);
	echo '<input name="origen" type="text" value="'.$des.'"/>';
}
?>
</div>
