...
if (!empty($refran)){
	...
	$origen = $_POST['origen'];
	$cadena = "UPDATE refranes SET des = '".$refran."', idorigen = ".$origen.", comentario = '".$comentario."', desm = '".$hrefran."', clave = '".$clave."' WHERE id = ".$refid;
	$resultado = $db->sql_query($cadena);
	$caddelrc = "DELETE FROM refranclasif WHERE idrefran = ".$refid;
	$resdelrc = $db->sql_query($caddelrc);
	$caddelrl = "DELETE FROM refranlocaliza WHERE idrefran = ".$refid;
	$resdelrl = $db->sql_query($caddelrl);
	...
	while ($i < $cantclasif) {
		...
		if (isset($_POST[$idclasif])){
			$cadena1 = "INSERT INTO refranclasif (idrefran, idclasif) VALUES('".$refid."', ".$idclasif.")";
	...
	if (isset($_SESSION['locaciones'])){
		...
		if ($total > 0){
			...
			while ($i < $total) {
				...
				$cantper = $actual['num'];
				$cadena2 = "INSERT INTO refranlocaliza (idrefran, idlocaliza, cantper) VALUES('".$refid."', ".$codigogeo.", ".$cantper.")";
	...
	if (isset($_SESSION['variantes'])){
		...
		$cadmaxv = "SELECT * FROM variante WHERE idrefran = ".$refid;
		$resmaxv = $db->sql_query($cadmaxv);
		$cantv = $db->sql_numrows($resmaxv);
		$i = 0;
		while ($i < $cantv) {
			$idv = $db->sql_fetchfield('id', $i, $resmaxv);
			$caddelvl = "DELETE FROM varlocaliza WHERE idvariante = ".$idv;
			$resdelvl = $db->sql_query($caddelvl);
			$i++;
		}
		$caddelv = "DELETE FROM variante WHERE idrefran = ".$refid;
		$resdelvl = $db->sql_query($caddelv);
		...
		while ($i < $totalvar) {
			$tempdes = $variantes->desvariante($varactual);
			if (!empty($tempdes)){
				$cadena3 = "INSERT INTO variante (idrefran, des) VALUES('".$refid."', '".$tempdes."')";
				...
				if (isset($locacionesvar)){
					...
					if ($totalloca > 0){
						...
						while ($j < $totalloca) {
							...
							$cadena4 = "INSERT INTO varlocaliza (idvariante, idrefran, idlocaliza, cantper) VALUES('".$idv."', '".$refid."', '".$idloca."', '".$locactual['num']."')";
	...
	header("Location: fdetallerefran.php?refid=".$refid);
}else{
