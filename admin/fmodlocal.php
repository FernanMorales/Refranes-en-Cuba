    ...
    <td><div id="main">
	 <div id="contenido">
	  <?php include('menu.php') ?>
	   <div id="titulo">Modificar c&oacute;digo geogr&aacute;fico 
	     <div id="logout"><a href="logout.php" class="elemento logout">Salir</a></div></div>
	   <div id="cajaizq">
	     <form action="modlocal.php" method="post" name="form" id="form">	       	   
<?php
if (isset($_GET['ref'])){
	$id = $_GET['ref'];
	echo '<input name="localid" type="hidden" value="'.$id.'" />';
	$cadref = "SELECT * FROM localizacion WHERE id = ".$_GET['ref'];
	...
	echo '<div id="cajainsert">';
	echo '<input name="codigo" type="text" value="'.$codigo.'"/>';
	echo '</div>';
	echo '<div class="titulo">Descripción</div>';
	echo '<div id="cajainsert">';
	echo '<input name="des" type="text" value="'.$des.'"/>';
	echo '</div>';	
}
?>
